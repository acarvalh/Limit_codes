...
for i in `echo ${runLimits[@]}`; do
	...
	sed -i "/TString hhiggsggh/c\TString hhiggsggh = \"$dir/${limitdirs[$i]}/ggh_m125_powheg_8TeV_m${imass}.root\"/" $fitterScript
	sed -i "/TString hhiggstth/c\TString hhiggstth = \"$dir/${limitdirs[$i]}/tth_m125_8TeV_m${imass}.root\"/" $fitterScript
	sed -i "/TString hhiggsvbf/c\TString hhiggsvbf = \"$dir/${limitdirs[$i]}/vbf_m125_8TeV_m${imass}.root\"/" $fitterScript
	sed -i "/TString hhiggsvh/c\TString hhiggsvh = \"$dir/${limitdirs[$i]}/wzh_m125_8TeV_zh_m${imass}.root\"/" $fitterScript
	sed -i "/TString hhiggsbbh/c\TString hhiggsbbh = \"$dir/${limitdirs[$i]}/bbh_m125_8TeV_m${imass}.root\"/" $fitterScript
	sed -i "/TString ddata/c\TString ddata = \"$dir/${limitdirs[$i]}/Data_m${imass}.root\"/" $fitterScript

	if [ ${imass} == "0" ]; then
	    sed -i "/TString ssignal/c\TString ssignal = \"$dir/${limitdirs[$i]}/ggHH_8TeV_m0.root\"/" $fitterScript
	elif [ ${imass} == "260" ]; then
	    sed -i "/TString ssignal/c\TString ssignal = \"$dir/${limitdirs[$i]}/MSSM_m260_8TeV_m260.root\"/" $fitterScript
	else
	    sed -i "/TString ssignal/c\TString ssignal = \"$dir/${limitdirs[$i]}/Radion_m${imass}_8TeV_${imass}.root\"/" $fitterScript
	fi

        # the legend
	sed -i -r -e "s/[0-9]+ GeV\"\);/${masses[$j]} GeV\"\);/g" $fitterScript

	echo "Running limits for $imass on ${limitdirs[$i]}"
