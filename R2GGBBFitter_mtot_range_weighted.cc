...
TString FinalCuts_cat0 = "";
TString FinalCuts_cat1 = "";
TString baseDir = "";
...
void runfits(const Float_t mass=120, Int_t mode=1, Bool_t dobands = false,TString CUTS_cat0,TString CUTS_cat1, TString BASEDIR)
{
  style();
  FinalCuts_cat0 = CUTS_cat0;
  FinalCuts_cat1 = CUTS_cat1;
  baseDir = BASEDIR;
...
void AddSigData(RooWorkspace* w, Float_t mass, TString signalfile) {
  ...
  TString cut0 = "&& mgg > 120 && mgg < 130";
  for (int c = 0; c < ncat; ++c) {
    if(c==0) sigToFit[c] = (RooDataSet*) sigScaled.reduce(
        *w->var("mtot"),
        mainCut+TString::Format(" && cut_based_ct==%d",c)+cut0+FinalCuts_cat0);
    if(c==1) sigToFit[c] = (RooDataSet*) sigScaled.reduce(
        *w->var("mtot"),
        mainCut+TString::Format(" && cut_based_ct==%d",c)+cut0+FinalCuts_cat1);
...
void AddBkgData(RooWorkspace* w, TString datafile) {
  ...
  for (int c = 0; c < ncat; ++c) {
    if(c==0) dataToFit[c] = (RooDataSet*) Data.reduce(
        *w->var("mtot"),
        TString::Format(" cut_based_ct==%d && mtot > %d",c,minfit)+cut0+FinalCuts_cat0);
    if(c==1) dataToFit[c] = (RooDataSet*) Data.reduce(
        *w->var("mtot"),
        TString::Format(" cut_based_ct==%d && mtot > %d",c,minfit)+cut0+FinalCuts_cat1);
    if(c==0) dataToPlot[c] = (RooDataSet*) Data.reduce(
        *w->var("mtot"),
        TString::Format(" cut_based_ct==%d && mtot > %d",c,minfit)+ cut0+FinalCuts_cat0);
    if(c==1) dataToPlot[c] = (RooDataSet*) Data.reduce(
        *w->var("mtot"),
        TString::Format(" cut_based_ct==%d && mtot > %d",c,minfit)+ cut0+FinalCuts_cat1);
...
cout<<"here"<<endl;
  ...
  TString filename_cat0(cardDir+TString(fileBaseName)+"rep_cat0.txt");
  ofstream outFile_cat0(filename_cat0);
  outFile_cat0 << "#CMS-HGG DataCard for Unbinned Limit Setting, " << lumi->getVal() << " pb-1 " << endl;
  outFile_cat0 << "#Run with: combine -d hgg.mH130.0.shapes-Unbinned.txt -U -m 130 -H ProfileLikelihood -M MarkovChainMC --rMin=0 --rMax=20.0 -b 3000 -i 50000 --optimizeSim=1 --tries 30" << endl;
  outFile_cat0 << "# Lumi = " << lumi->getVal() << " pb-1" << endl;
  outFile_cat0 << "imax 1" << endl;
  outFile_cat0 << "jmax 1" << endl;
  outFile_cat0 << "kmax *" << endl;
  outFile_cat0 << "---------------" << endl;
cout<<"here"<<endl;
  outFile_cat0 << "# the name after w_all is the name of the rooextpdf we want to use, we have both saved" << endl;
  outFile_cat0 << "# BKG" << endl;
  ...
  outFile_cat0 << "shapes mtotBkg cat0 " << TString(fileBkgName)+".root" << " w_all:CMS_hgg_bkg_8TeV_cat0" << endl;
  outFile_cat0 << "shapes mtotBkg cat1 "<< TString(fileBkgName)+".root" << " w_all:CMS_hgg_bkg_8TeV_cat1" << endl;
  outFile_cat0 << "# signal" << endl;
  outFile_cat0 << "shapes mtotSig cat0 " << TString(fileBaseName)+".inputsig.root" << " w_all:CMS_hgg_sig_cat0" << endl;
  outFile_cat0 << "shapes mtotSig cat1 " << TString(fileBaseName)+".inputsig.root" << " w_all:CMS_hgg_sig_cat1" << endl;
  outFile_cat0 << "---------------" << endl;
  /////////////////////////////////////
  /////////////////////////////////////
  outFile_cat0 << "bin cat0 " << endl;
  outFile_cat0 << "observation "
        << data[0]->sumEntries() << " "
        << endl;
  outFile_cat0 << "------------------------------" << endl;
  outFile_cat0 << "bin cat0 cat0" << endl;
  outFile_cat0 << "process mtotSig mtotBkg" << endl;
  outFile_cat0 << "process 0 1" << endl;
  outFile_cat0 << "rate "
         << " " << sigToFit[0]->sumEntries() << " " << 1
         << " " << endl;
  outFile_cat0 << "--------------------------------" << endl;
  ...
  outFile_cat0 << "DiphoTrigger lnN "
        << "1.01 - "
        << "# Trigger efficiency" << endl;
  outFile_cat0 << "############## jet" << endl;
  outFile_cat0 << "Extra_Photon_selectios_accep              lnN " 
	<< "1.050        -   "
	<<"# photon acceptance " << endl;
  outFile_cat0 << "Photon_selectios_accep              lnN " 
	<< "1.010        -   "
	<<"# photon acceptance = sqrt(0.8^2 +5^2)" << endl;
  outFile_cat0 << "Mjj_acceptance lnN "
        << "1.015 - "
        <<"# JER and JES " << endl;
  outFile_cat0 << "btag_eff lnN "
        << "1.0527 - "
        <<"# b tag efficiency uncertainty" << endl;
  outFile_cat0 << "############## photon " << endl;
  outFile_cat0 << "CMS_hgg_eff_g lnN "
          << "1.010 - "
          << "# photon selection accep." << endl;
  outFile_cat0 << "############## for mtot fit" << endl;
  outFile_cat0 << "maa_acceptance lnN "
          << "1.005 - "
          << "# photon energy resolution" << endl;
  outFile_cat0 << "############## normalization floating" << endl;
  outFile_cat0 << "# Parametric shape uncertainties, entered by hand. they act on both higgs/signal " << endl;
  outFile_cat0 << "CMS_hgg_sig_m0_absShift param 1 0.014 # displacement of the dipho mean (0.45 + 0.8 + 1.0) in quadrature" << endl;
  outFile_cat0 << "CMS_hgg_sig_sigmaScale param 1 0.10 # optimistic estimative of resolution uncertainty " << endl;
  outFile_cat0 << "############## for mtot fit - slopes" << endl;
  outFile_cat0 << "############## with reparametrization" << endl;
  ...
  cout << "Write data card in: " << filename_cat0 << " file" << endl;
    ...
    TString filename_cat1(cardDir+TString(fileBaseName)+"rep_cat1.txt");
  ofstream outFile_cat1(filename_cat1);
  outFile_cat1 << "#CMS-HGG DataCard for Unbinned Limit Setting, " << lumi->getVal() << " pb-1 " << endl;
  outFile_cat1 << "#Run with: combine -d hgg.mH130.0.shapes-Unbinned.txt -U -m 130 -H ProfileLikelihood -M MarkovChainMC --rMin=0 --rMax=20.0 -b 3000 -i 50000 --optimizeSim=1 --tries 30" << endl;
  outFile_cat1 << "# Lumi = " << lumi->getVal() << " pb-1" << endl;
  outFile_cat1 << "imax 1" << endl;
  outFile_cat1 << "jmax 1" << endl;
  outFile_cat1 << "kmax *" << endl;
  outFile_cat1 << "---------------" << endl;
cout<<"here"<<endl;
  outFile_cat1 << "# the name after w_all is the name of the rooextpdf we want to use, we have both saved" << endl;
  outFile_cat1 << "# BKG" << endl;
  ...
  outFile_cat1 << "shapes mtotBkg cat0 " << TString(fileBkgName)+".root" << " w_all:CMS_hgg_bkg_8TeV_cat0" << endl;
  outFile_cat1 << "shapes mtotBkg cat1 "<< TString(fileBkgName)+".root" << " w_all:CMS_hgg_bkg_8TeV_cat1" << endl;
  outFile_cat1 << "# signal" << endl;
  outFile_cat1 << "shapes mtotSig cat0 " << TString(fileBaseName)+".inputsig.root" << " w_all:CMS_hgg_sig_cat0" << endl;
  outFile_cat1 << "shapes mtotSig cat1 " << TString(fileBaseName)+".inputsig.root" << " w_all:CMS_hgg_sig_cat1" << endl;
  outFile_cat1 << "---------------" << endl;
  /////////////////////////////////////
  /////////////////////////////////////
  outFile_cat1 << "bin cat1 " << endl;
  outFile_cat1 << "observation "
        << data[1]->sumEntries() << " "
        << endl;
  outFile_cat1 << "------------------------------" << endl;
  outFile_cat1 << "bin cat1 cat1" << endl;
  outFile_cat1 << "process mtotSig mtotBkg " << endl;
  outFile_cat1 << "process 0 1 " << endl;
  outFile_cat1 << "rate "
         << " " << sigToFit[1]->sumEntries() << " " << 1
         << " " << endl;
  outFile_cat1 << "--------------------------------" << endl;
  ...
  outFile_cat1 << "DiphoTrigger lnN "
        << "1.01 - "
        << "# Trigger efficiency" << endl;
  outFile_cat1 << "############## jet" << endl;
  outFile_cat1 << "Extra_Photon_selectios_accep              lnN "
	<< "1.050        -   "
	<<"# photon acceptance " << endl;
  outFile_cat1 << "Photon_selectios_accep              lnN " 
	<< "1.010        -   "
	<<"# photon acceptance = sqrt(0.8^2 +5^2)" << endl;
  outFile_cat1 << "Mjj_acceptance lnN "
        << "1.015 - "
        <<"# JER and JES " << endl;
  outFile_cat1 << "btag_eff lnN "
        << "0.9818 - "
        <<"# b tag efficiency uncertainty" << endl;
  outFile_cat1 << "############## photon " << endl;
  outFile_cat1 << "CMS_hgg_eff_g lnN "
          << "1.010 - "
          << "# photon selection accep." << endl;
  outFile_cat1 << "############## for mtot fit" << endl;
  outFile_cat1 << "maa_acceptance lnN "
          << "1.005 - "
          << "# photon energy resolution" << endl;
  outFile_cat1 << "############## normalization floating" << endl;
  outFile_cat1 << "# Parametric shape uncertainties, entered by hand. they act on both higgs/signal " << endl;
  outFile_cat1 << "CMS_hgg_sig_m0_absShift param 1 0.014 # displacement of the dipho mean (0.45 + 0.8 + 1.0) in quadrature" << endl;
  outFile_cat1 << "CMS_hgg_sig_sigmaScale param 1 0.10 # optimistic estimative of resolution uncertainty " << endl;
  outFile_cat1 << "############## for mtot fit - slopes" << endl;
  outFile_cat1 << "############## with reparametrization" << endl;
  ...
  outFile_cat1 << "CMS_hgg_bkg_8TeV_slope2_cat1 flatParam # Mean and absolute uncertainty on background slope" << endl;
  outFile_cat1.close();
