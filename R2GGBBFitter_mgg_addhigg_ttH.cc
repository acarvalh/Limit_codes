...
{
  style();
  TString fileBaseName(TString::Format("hgg.hig.mH%.1f_8TeV.tth", mass));
...
minSigFit+1.5,0.75*plotmgg[c]->GetMaximum(),catdesc.at(c));
    ...
    //latex -> Draw("same");
    ctmp->SaveAs(TString::Format("sigmodel_tth_cat%d.pdf",c));
    ctmp->SaveAs(TString::Format("sigmodel_tth_cat%d.png",c));
